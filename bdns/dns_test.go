...
func TestDNSNoServers(t *testing.T) {
	...
	_, err := obj.LookupA(context.Background(), "letsencrypt.org")
...
func TestDNSServFail(t *testing.T) {
	...
	_, err = obj.LookupA(context.Background(), bad)
...
func TestDNSLookupA(t *testing.T) {
	...
	ip, err := obj.LookupA(context.Background(), "servfail.com")
	t.Logf("servfail.com - IP: %s, Err: %s", ip, err)
	test.AssertError(t, err, "Server failure")
	test.Assert(t, len(ip) == 0, "Should not have IPs")

	ip, err = obj.LookupA(context.Background(), "nonexistent.letsencrypt.org")
	t.Logf("nonexistent.letsencrypt.org - IP: %s, Err: %s", ip, err)
	test.AssertNotError(t, err, "Not an error to not exist")
	test.Assert(t, len(ip) == 0, "Should not have IPs")

	// Single IPv4 address
	ip, err = obj.LookupA(context.Background(), "cps.letsencrypt.org")
	...
	// No IPv6
	ip, err = obj.LookupA(context.Background(), "v6.letsencrypt.org")
