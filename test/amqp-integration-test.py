...
def make_ocsp_req(cert_file):
    ...
    cmd = ("openssl ocsp -no_nonce -issuer ../test-ca.pem -cert %s -reqout %s" % (
        cert_file, ocsp_req_file))
    print cmd
...
def fetch_until(cert_file, url, initial, final):
    ...
    """
    ocsp_request = make_ocsp_req(cert_file)
    timeout = time.time() + 5
    while True:
        time.sleep(0.25)
        ...
        for resp in responses:
            verify_output = ocsp_verify(cert_file, resp)
...
def ocsp_verify(cert_file, ocsp_response):
...
def wait_for_ocsp_good(cert_file, url):
    fetch_until(cert_file, url, " unauthorized", ": good")

def wait_for_ocsp_revoked(cert_file, url):
    fetch_until(cert_file, url, ": good", ": revoked")
...
def run_node_test():
    ...
    wait_for_ocsp_revoked(cert_file_pem, ee_ocsp_url)
    return 0
